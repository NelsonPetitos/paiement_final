...
router.get('/operators/:countryid', function(req, res){
    if(req.dburl){
        pg.connect(req.dburl, function(err, client, done) {
            ...
            client.query('SELECT name FROM phone_operators WERE country_id = $1', [req.params.countryid], function(err, result) {
                done();
                if(err){ 
                ...
                res.status(200).json({ err: false, msg: 'User create.', data: result.rows });
            });
        });
    }else{
